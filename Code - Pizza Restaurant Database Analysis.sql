...
create database PizzaRestaurantDatabase
GO

-- Selecting the Database.
use PizzaRestaurantDatabase
GO
...
-- Number of Pizza's varieties sold by the restraunt.
select COUNT(distinct Pizza_Name) as Pizza_Variety_Count from Pizza_Types
GO

-- Number of Pizza's currently in each category.
select Category, COUNT(Pizza_Name) as Number_Of_Pizzas from Pizza_Types
group by Category
order by Category desc
GO

-- Number of ingredients used in each pizza.
select Ingredients, LEN(Ingredients) - LEN(REPLACE(Ingredients, ',', '')) + 1 AS Number_Of_Ingredients -- Replaced the commas with an empty string, added 1 to that value and subtracted this length with the original Ingridient column length.
from Pizza_Types
GO

-- The average number of ingredients used to make one single pizza.
...
-- The top most and least used ingredients in the Pizza's.
...
-- The average price of Pizzas based on Size.
select Size, 
case when Size = 'S' 
...
-- Average price of all pizzas regardless of size.
select format(AVG(Price), 'C', 'en-US') as Avg_Pizza_Price from Pizza
GO

-- Average price of pizzas under each category.
...
-- Check whether all sizes are available for each Pizza type.
...
-- The most expensive small, medium, large, X large and XX large pizza.
...
-- Total number of pizzas sold in 2015.
select SUM(Quantity) as Numnber_Of_Pizzas_Sold from Order_Details
GO

-- Least popular to most popular pizza category. Based on number of orders.
select case when pt.Category is null then 'TOTAL = ' else pt.Category end Category, 
	   SUM(Quantity) as Pizza_Order_Count from Order_Details od
...
order by Pizza_Order_Count asc
GO

-- Top 5 best selling pizzas and top 5 least selling pizzas. Based on number of orders.
...
-- Most popular pizza size based. Based on number of orders.
...
-- Number of orders received in 2015.
select COUNT(Order_ID) as Number_Of_Orders from Orders
GO

-- Average number of orders received per day.
...
-- Number of orders received each day of 2015.
...
-- The number of orders received on each week day.
...
-- The number of orders received each month.
...
-- Number of orders received each quarter.
...
-- Total number of orders received in each hour in 2015.
...
-- Number of orders received based on brunch, lunch, snack and dinner.
...
-- Average number of pizzas sold each day.
...
select AVG(Total_Pizzas_Sold) from Avg_Pizza_Sold_Per_Day
GO

-- Average number of pizzas sold each hour.
select tbl1.* from 
(
select DATEPART(DAYOFYEAR, o.Order_Date) as dayofyear, DATEPART(HOUR, o.Order_Time) as hourofday, 
sum(od.Quantity) over(partition by DATEPART(DAYOFYEAR, o.Order_Date) order by DATEPART(HOUR, o.Order_Time)) as Summed,
ROW_NUMBER() over(partition by DATEPART(DAYOFYEAR, o.Order_Date), DATEPART(HOUR, o.Order_Time) order by (select null)) as rownum
from Orders o
inner join Order_Details od on o.Order_ID = od.Order_ID
-- where DATEPART(HOUR, o.Order_Time) = 17
-- order by dayofyear, hourofday
) tbl1
-- select hourofday, avg(sum(Summed)) as AvgOfSummed from test
-- where rownum = 1
group by tbl1.hourofday, rownum
GO
...
-- Average cart value of a customer.
...
select DATEPART(DAYOFYEAR, o.Order_Date), SUM(od.Quantity) from Orders o
inner join Order_Details od on o.Order_ID = od.Order_ID
group by rollup(DATEPART(DAYOFYEAR, o.Order_Date))

-- Total revenue generated from selling Pizzas in 2015.
select format(SUM(od.Quantity * p.Price), 'C', 'en-US') as Revenue_2015 from Order_Details od
inner join Pizza p on od.Pizza_ID = p.Pizza_ID
GO

-- Total revenue generated from each pizza category.
...
-- Total revenue generated based on pizza size.
...
-- Best selling and worst selling pizzas, based on revenue.
select od.Pizza_ID, pt.Category, pt.Pizza_Name, SUM(p.Price * od.Quantity) as Revenue_Generated,
